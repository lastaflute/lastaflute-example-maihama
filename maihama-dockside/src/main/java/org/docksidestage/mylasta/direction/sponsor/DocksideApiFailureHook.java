...
import org.docksidestage.mylasta.direction.sponsor.DocksideApiFailureHook.UnifiedFailureResult.FailureErrorPart;
 ...
 * @author jflute
 */
public class DocksideApiFailureHook extends MaihamaApiFailureHook { // #change_it for handling API failure
    ...
    protected List<FailureErrorPart> toErrors(Map<String, List<String>> propertyMessageMap) {
        return propertyMessageMap.entrySet().stream().map(entry -> {
            return new FailureErrorPart(entry.getKey(), entry.getValue());
    ...
    public static class UnifiedFailureResult {
        ...
        public static class FailureErrorPart {
            ...
            @NotNull
            public final List<String> messages; // as server-managed message way
